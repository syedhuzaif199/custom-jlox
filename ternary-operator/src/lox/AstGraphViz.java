...
class AstGraphViz implements Expr.Visitor<String>{
    ...
    public String visitTernaryExpr(Ternary expr) {
        String nodeId = "node" + System.identityHashCode(expr);
        builder.append(String.format("%s [label=\"%s\"];\n", nodeId, expr.operator1.lexeme + expr.operator2.lexeme));
        String firstId = expr.first.accept(this);
        String secondId = expr.second.accept(this);
        String thirdId = expr.third.accept(this);
        builder.append(String.format("%s -> %s;\n", nodeId, firstId));
        builder.append(String.format("%s -> %s;\n", nodeId, secondId));
        builder.append(String.format("%s -> %s;\n", nodeId, thirdId));
        return nodeId;
